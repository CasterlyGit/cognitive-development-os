...
class PrivateDataPolicyTests(unittest.TestCase):
    ...
    def test_quarantine_plan_is_exact_deterministic_and_effect_free(self):
        ...
        first = self.engine.plan_deletion(request)
        second = self.engine.plan_deletion(request)
        ...
        with self.assertRaises(DataPolicyError):
            self.engine.plan_deletion(replace(request, mode=DeletionMode.PURGE))
        with self.assertRaises(DataPolicyError):
            self.engine.plan_deletion(replace(request, source_ids=("source_*",)))
