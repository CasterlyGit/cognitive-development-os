...
| Semantic extraction with confidence | **Implemented — review gate** | Issue #20 review branch adds typed deterministic bands/scores/signals, hedged-action fallback to exploration, historical `unassessed` replay, and tests that confidence cannot confirm intent. It has no model/API inference. |
...
This is the **first next execution path** after the review gates:
...
Issue #20 implements the next independently reviewable slice: typed confidence
