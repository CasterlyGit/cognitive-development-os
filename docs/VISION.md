...
| Fixed two-project decision-loop MVP | **Implemented — review gate** | Issue #36 binds exactly two opted-in scopes, one source-backed relationship decision, a dependency-closed draft plan, route test doubles, and a local evidence timeline. It grants no execution authority and does not complete the general multi-project field. |
